        ...
        <div class="flex justify-start">
            <div class="w-full lg:w-full bg-white p-8 rounded-xl mx-3">
                @if($apply_data == null)
                    ...
                    {{-- Back button --}}
                    <div class="flex justify-center">
                        <a href="{{ route('user.apply') }}" class="bg-blue-700 hover:bg-blue-800 text-white font-bold py-2 px-4 rounded-md mt-3">Apply Now</a>
                    </div>
                @else
                    @if (Session('success'))
                            ...
                            <div class="p-4 px-4 md:p-8 mb-6">
                                <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
                                    <div class="text-gray-600">
                                        <p class="text-lg font-bold">{{ $apply_data->document->first_name . $apply_data->document->family_name }} Profile Details</p>
                                    </div>
                                    <div class="lg:col-span-2">
                                        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-6">
                                            <div class="md:col-span-3">
                                                <label for="full_name">First Name</label>
                                                <input type="text" name="full_name" id="full_name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ $apply_data->document->first_name }}" disabled/>
                                            </div>
                                            ...
                                            <div class="md:col-span-3">
                                                ...
                                                <input type="text" name="phone" id="phone" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ $apply_data->document->phone_number }}" placeholder="" disabled/>
                                            </div>
                        
                                            <div class="md:col-span-3">
                                                <label for="nationality">Nationality</label>
                                                <input type="text" name="nationality" id="nationality" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ $apply_data->document->nationality }}" placeholder="" disabled/>
                                            </div>
                        
                                            <div class="md:col-span-3">
                                                <label for="passport_nummber">Passport Number</label>
                                                <input type="text" name="passport_nummber" id="passport_nummber" class="transition-all flex items-center h-10 border mt-1 rounded px-4 w-full bg-gray-50" placeholder="" value="{{ $apply_data->document->passport_number }}" disabled/>
                                            </div>

                                            <div class="md:col-span-3">
                                                <label for="department">Department</label>
                                                <input type="text" name="department" id="department" class="transition-all flex items-center h-10 border mt-1 rounded px-4 w-full bg-gray-50" placeholder="" value="{{ $apply_data->document->department }}" disabled/>
                                            </div>
                                            {{-- File --}}
                                            @php
                                                $documents = [
                                            ...
                                            @if (!empty($apply_data->document))
                                                @foreach ($documents as $key => $label)
                                                    ...
                                                    <div class="md:col-span-2 flex items-center bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
                                                        <svg class="w-3.5 h-3.5 me-2 text-green-500 dark:text-green-400 flex-shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                                            <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 8.207-4 4a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L9 10.586l3.293-3.293a1 1 0 0 1 1.414 1.414Z"/>
                                                        </svg>
                                                        <a href="#" @click.prevent="openModal('{{ asset('storage/' . $apply_data->document->$key) }}')" class="text-base text-blue-500 hover:underline">
                                                            {{ $label }}
                                                        </a>
                                                    </div>
                                                    @endif
                    ...
                    {{-- <div class="flex justify-center">
                        <div class="w-full lg:w-1/2 bg-white p-8 rounded-xl mx-3">
                            <div class="flex justify-center">
                                <h1 class="text-2xl font-bold">Scholarship Application</h1>
                            </div>
                            <!-- Card Data Diri -->
                            <div class="bg-gray-100 p-6 rounded-lg shadow-md justify-start">
                                <h1 class="text-2xl font-bold mb-4">{{ $apply_data->document->first_name }} {{ $apply_data->document->family_name }}'s Details</h1>
                                <div class="mb-4 space-y-4">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <input type="text" class="bg-gray-100 border-0 text-gray-800 text-lg font-medium" value="{{ $apply_data->document->email }}" readonly>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <input type="text" class="bg-gray-100 border-0 text-gray-800 text-lg font-medium" value="{{ $apply_data->document->phone_number }}" readonly>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <input type="text" class="bg-gray-100 border-0 text-gray-800 text-lg font-medium" value="{{ $apply_data->document->nationality }}" readonly>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <input type="text" class="bg-gray-100 border-0 text-gray-800 text-lg font-medium" value="{{ $apply_data->document->passport_number }}" readonly>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <input type="text" class="bg-gray-100 border-0 text-gray-800 text-lg font-medium" value="{{ $apply_data->document->department }}" readonly>
                                    </div>
                                </div>
                            </div>
