    ...
    <div class="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg mt-10">
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-6">
            ...
            <div class="bg-gray-100 p-6 rounded-lg shadow-md">
                <h1 class="text-2xl font-bold mb-4">{{ $applicant->document->first_name }} {{ $applicant->document->family_name }}'s Details</h1>
                <div class="mb-4 space-y-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <input type="text" class="bg-gray-100 border-0 text-gray-800 text-lg font-medium" value="{{ $applicant->document->email }}" readonly>
                    </div>
                    <div class="flex items-center space-x-2">
                        ...
                        <input type="text" class="bg-gray-100 border-0 text-gray-800 text-lg font-medium" value="{{ $applicant->document->phone_number }}" readonly>
                    </div>
                    <div class="flex items-center space-x-2">
                        ...
                        <input type="text" class="bg-gray-100 border-0 text-gray-800 text-lg font-medium" value="{{ $applicant->document->nationality }}" readonly>
                    </div>
                    <div class="flex items-center space-x-2">
                        ...
                        <input type="text" class="bg-gray-100 border-0 text-gray-800 text-lg font-medium" value="{{ $applicant->document->passport_number }}" readonly>
                    </div>
                    <div class="flex items-center space-x-2">
                        ...
                        <input type="text" class="bg-gray-100 border-0 text-gray-800 text-lg font-medium" value="{{ $applicant->document->department }}" readonly>
                    </div>
                </div>
            </div>
                ...
                @if (!empty($applicant))
                    <ul class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach ($documents as $key => $label)
                            @if (!empty($applicant->document->$key))
                                <li class="flex items-center bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
                                    ...
                                    <a href="#" @click.prevent="openModal('{{ asset('storage/' . $applicant->document->$key) }}')" class="text-base text-blue-500 hover:underline">
                                        {{ $label }}
                                    </a>
                                </li>
                            @endif
